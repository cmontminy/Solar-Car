...
CAN_Interrupt_Handler can1(PA_11, PA_12, 3);     // Address 0x21, priority 3
...
//------------------------------------------------------------------------------
// Filtering Test Code (Change to main to test) (For Receiver)
int main(){
    setup();
    //can1.setMode(0);
    //can1.setMode(1);
    pcConnection.printf("Test\n\r");
    can1.setMode(1, 1337);
    while(1){
        if(!can1.isFIFOEmpty()){
...
//------------------------------------------------------------------------------
// Filtering Test Code (Change to main to test) (For Trasmitter)
/*int main(){
    setup();
    //can1.setMode(0);
    //can1.setMode(1);
    //can1.setMode(2, 0x5);
    pcConnection.printf("Test/n/r");
    led1 = 1;
    sendTestMessage();
    led1 = 0;
    pcConnection.printf("Finished/n/r");*/
    /*
    while(1){
        if(!can1.isFIFOEmpty()){
            can1.getNextMessage(receiverMailBox);
